...
void ST7789_Fill(uint16_t xSta, uint16_t ySta, uint16_t xEnd, uint16_t yEnd, uint16_t color)
{
	...
	uint32_t pixels = ((uint32_t)(xEnd - xSta + 1U)) * ((uint32_t)(yEnd - ySta + 1U));
	uint8_t data[128];
	uint16_t chunkPixels;
	uint16_t maxChunkPixels = sizeof(data) / 2U;

	for (uint16_t i = 0U; i < maxChunkPixels; i++) {
		data[i * 2U] = color >> 8;
		data[(i * 2U) + 1U] = color & 0xFF;
	}

	ST7789_SetAddressWindow(xSta, ySta, xEnd, yEnd);
	while (pixels > 0U) {
		chunkPixels = (pixels > maxChunkPixels) ? maxChunkPixels : (uint16_t)pixels;
		ST7789_WriteData(data, (size_t)chunkPixels * 2U);
		pixels -= chunkPixels;
	}
}
